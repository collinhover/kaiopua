...
var KAIOPUA = (function (main) {
    ...
    function instantiate ( parameters, instance ) {
		...
		// add bounds/dimensions functions
		
		instance.dimensions_from_bounding_box_scaled = dimensions_from_bounding_box_scaled;
		
		instance.center_offset_from_bounding_box = center_offset_from_bounding_box;
		...
		if ( true ) {//parameters.adjustForOffset === true ) {
			
			var dimensions = dimensions_from_bounding_box_scaled( mesh ),
				centerOffset = center_offset_from_bounding_box( mesh );
			
			console.log('--------ADJUSTING FOR OFFSET-------');
			console.log( ' mesh for model has following offset: ' );
			console.log( 'dimensions: ', dimensions.x, dimensions.y, dimensions.z );
			console.log( 'center offset: ', centerOffset.x, centerOffset.y, centerOffset.z );
			console.log('-----------------------------------');
			...
			for ( i = 0, l = vertices.length; i < l; i ++) {
				...
				vertexPosition.subSelf( centerOffset );
			...
			mesh.position.addSelf( centerOffset );
	...
	function dimensions_from_bounding_box_scaled( mesh ) {
		...
		if ( bbox ) {
			...
			dimensions.set( bbox.max.x - bbox.min.x, bbox.max.y - bbox.min.y, bbox.max.z - bbox.min.z ).multiplySelf( scale );
	...
	function center_offset_from_bounding_box ( mesh ) {
		...
		var geometry = mesh.geometry,
			centerOffset = dimensions_from_bounding_box_scaled( mesh );
			bbox = geometry.boundingBox;
		
		if ( bbox ) {
			
			centerOffset.multiplyScalar( 0.5 ).addSelf( bbox.min );
